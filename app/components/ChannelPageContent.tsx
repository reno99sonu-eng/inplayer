...
import type { PublicProfileData, PublicProfileVideo } from "@/app/lib/getPublicProfile";
import { formatTimeAgo, formatViews, formatCompactNumber } from "@/app/lib/formatters";
...
function sortVideos(videos: PublicProfileVideo[], sort: Sort) {
...
// Server-rendered by app/u/[username]/page.tsx, which resolves this exact
// same profile ANONYMOUSLY (a Next.js Server Component has no
// Authorization header to read, so it always asks for viewerId: null) —
// that's what makes the very first paint real HTML instead of the blank
// spinner this page used to show while JS booted and its first client
// fetch round-tripped. A signed-in visitor's browser re-fetches its own
// AUTHENTICATED view once mounted (below) — that's the only way to learn
// isOwner, or to unlock a connections-gated channel, since the server
// can't see the client's Cognito Bearer token. A signed-OUT visitor skips
// that re-fetch entirely: the server-rendered anonymous data is already
// their correct, final view.
export default function ChannelPageContent({ username, initialProfile }: ChannelPageContentProps) {
  const router = useRouter();
  const { signedIn, user } = useAuthModal();
  const [profile, setProfile] = useState<PublicProfileData>(initialProfile);
  ...
  useEffect(() => {
    ...
    async function loadAuthenticated() {
      try {
        ...
        console.error("Failed to load authenticated channel view:", error);
      }
    }
  ...
  // Split strictly on the real contentType set at upload time — NOT on the
  // freely-chosen topical `category` field (which happens to include a
  // "Raftaar (Vertical Videos)" option a creator can tag a regular
  // long-form video with). Matching on category too used to leak
  ...
  const videos: Recommendation[] = regularVideos.map((video) => ({ id: video.videoId, videoId: video.videoId, title: video.title, creator: profile.name || profile.username, uploaderUsername: profile.username, avatar: profile.avatarUrl || "/avatars/avatar.png", thumbnail: video.thumbnailUrl || "/recommendations/thumbnails/1.jpg", views: `${formatViews(video.views || 0)} views`, likes: `${formatCompactNumber(video.likeCount || 0)} likes`, uploaded: formatTimeAgo(video.uploadedAt), duration: "Video", verified: profile.isVerified, muxPlaybackId: video.muxPlaybackId }));
  const shorts: Short[] = shortVideos.map((video) => ({ id: video.videoId, videoId: video.videoId, title: video.title, creator: profile.name || profile.username, poster: video.thumbnailUrl || "/shorts/1.jpg", views: `${formatViews(video.views || 0)} views`, likes: `${formatCompactNumber(video.likeCount || 0)} likes`, comments: `${formatCompactNumber(video.commentCount || 0)} comments`, uploaderId: profile.userId, uploaderUsername: profile.username, uploaderAvatarUrl: profile.avatarUrl || undefined, muxPlaybackId: video.muxPlaybackId }));
  ...
  return <main className="mx-auto max-w-[1800px] px-4 py-4 [&>section>div:last-child]:!pt-20 sm:px-6 sm:py-5 sm:[&>section>div:last-child]:!pt-28 lg:px-8 lg:[&>section>div:last-child]:!pt-32">
    ...
    <section className="relative isolate overflow-hidden rounded-3xl border border-white/10 bg-[#0a1020] shadow-[0_20px_60px_rgba(0,0,0,.28)] light:border-black/10 light:bg-[#efe6d0]">
      <div className="absolute inset-0">{bannerImage ? <Image src={bannerImage} alt="" fill priority unoptimized sizes="100vw" className="object-cover opacity-85" /> : <div className="h-full bg-[radial-gradient(circle_at_20%_20%,rgba(249,115,22,.48),transparent_32%),radial-gradient(circle_at_80%_5%,rgba(251,191,36,.24),transparent_25%),linear-gradient(120deg,#10182d,#030712)]" />}<div className="absolute inset-0 bg-gradient-to-r from-[#060a14]/75 via-[#060a14]/40 to-transparent light:from-[#f4ecda]/75 light:via-[#f4ecda]/40 light:to-transparent" /></div>
