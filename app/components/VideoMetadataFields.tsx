    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {aiError && <p className="mb-1.5 text-xs text-red-400">{aiError}</p>}
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-[#060D18] px-3.5 py-2.5 text-xs text-white caret-orange-400 outline-none focus:border-orange-400/50 light:border-black/10 light:bg-white light:text-slate-900 sm:text-sm"
        ...
        {/* Description Field */}
        <div>
          <label className="mb-1.5 block text-xs font-bold text-slate-300 light:text-slate-700">
            Description
          </label>
          <textarea
            rows={3}
            ...
            className="w-full resize-none rounded-xl border border-white/10 bg-[#060D18] px-3.5 py-2 text-xs text-white caret-orange-400 outline-none focus:border-orange-400/50 light:border-black/10 light:bg-white light:text-slate-900 sm:text-sm"
          ...
          <div className="rounded-xl border border-white/10 bg-[#060D18] p-3 light:border-black/10 light:bg-white space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div
                ...
                role="button"
                className="group relative flex aspect-video h-14 cursor-pointer items-center justify-center overflow-hidden rounded-xl border border-white/15 bg-black/20"
              >
              ...
              {muxFrames.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white transition hover:bg-white/10 light:border-black/10 light:text-slate-800 disabled:opacity-50"
                >
                  <Sparkles size={14} className="text-orange-400" />
                  <span>{aiThumbBusy ? "Analyzing..." : "AI Pick Frame"}</span>
                </button>
              )}
            </div>
      ...
      {/* RIGHT COLUMN: Category, Visibility, Audience, Tags & Toggles */}
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl border border-white/10 bg-[#060D18] px-3 py-2 text-xs text-white outline-none focus:border-orange-400/50 light:border-black/10 light:bg-white light:text-slate-900"
            >
              {categories.map((c) => (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 rounded-xl border border-white/10 bg-[#060D18] p-2 light:border-black/10 light:bg-white">
            {value.tags.map((t) => (
              <span
                key={t}
                className="flex items-center gap-1 rounded-md bg-orange-500/15 px-2 py-0.5 text-[11px] font-semibold text-orange-300 light:text-orange-700"
              >
                #{t}
                <button
                  type="button"
            ...
            <input
              ...
              onBlur={addTag}
              placeholder={value.tags.length === 0 ? "Add tag and press Enter" : ""}
              className="min-w-[100px] flex-1 bg-transparent px-1 py-0.5 text-xs text-white outline-none placeholder:text-slate-500 light:text-slate-900"
            />
          </div>
        </div>
