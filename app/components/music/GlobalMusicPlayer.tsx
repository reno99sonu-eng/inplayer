...
export default function GlobalMusicPlayer() {
  ...
  if (currentTrack && currentTrack.videoId !== prevTrackId) {
    setPrevTrackId(currentTrack.videoId);
    setIsLiked(false);
    setLikeCount(
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="fixed inset-0 z-[120] flex flex-col bg-[#050B14]/95 text-white backdrop-blur-3xl animate-in fade-in zoom-in-95 duration-200 light:bg-[#FAF6EF]/95 light:text-slate-900 overflow-y-auto">
          ...
          <main className="relative z-10 flex-1 max-w-5xl mx-auto w-full px-6 py-6 flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-14">
            {isAdActive && currentAd ? (
              <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-14 w-full">
                {/* Left: Ad Creative Poster */}
                <div className="flex flex-col items-center max-w-sm w-full">
                  <div className="relative group w-64 h-64 sm:w-80 sm:h-80 rounded-3xl overflow-hidden shadow-[0_20px_60px_rgba(245,158,11,0.3)] border border-amber-500/40 bg-black">
                    <Image
                      src={currentAd.imageUrl || currentAd.creativeUrl || "/recommendations/thumbnails/1.jpg"}
                      alt={currentAd.title}
                      fill
                      sizes="(max-width: 768px) 320px, 400px"
                      className="object-cover"
                    />
                    <div className="absolute top-3 left-3 px-2.5 py-1 rounded-md bg-amber-500 text-black text-xs font-black uppercase tracking-wider shadow">
                      Sponsored Break
                    </div>
                  </div>
                  ...
                  <div className="mt-6 text-center w-full">
                    ...
                    <p className="text-sm font-semibold text-amber-400 truncate mt-0.5">
                      {currentAd.sponsorName || "InPlayer Sponsor"}
                    </p>
                  </div>
                </div>

                {/* Right: Ad Actions & Countdown */}
                <div className="flex-1 w-full max-w-md flex flex-col justify-center space-y-5 bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-semibold w-fit">
                    ...
                    <span>Listening Session • Track #{sessionTrackCount}</span>
                  </div>

                  <div>
                    <h2 className="text-xl font-bold text-white">Short Sponsor Break</h2>
                    <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                      Free listening session. Your music will resume shortly, or you can skip once the countdown expires.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
                    {skipUnlocked ? (
                      <button
                        onClick={skipAd}
                        className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-bold text-sm shadow-[0_0_20px_rgba(249,115,22,0.5)] transition active:scale-95"
                      >
                        Skip Ad →
                      </button>
                    ) : (
                      <div className="w-full sm:w-auto px-6 py-3 rounded-xl bg-black/60 border border-white/20 text-slate-300 font-semibold text-sm flex items-center justify-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                        <span>Skip in {adCountdown}s</span>
                      </div>
                    )}

                    {(currentAd.linkUrl || currentAd.clickUrl) && (
                      <a
                        href={currentAd.linkUrl || currentAd.clickUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={trackAdClick}
                        className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-white font-semibold text-sm transition border border-white/15"
                      >
                  ...
                  <div className="pt-4 border-t border-white/10 text-xs text-slate-400 flex items-center justify-between">
                    <span>Up next: <strong className="text-white">{currentTrack.title}</strong></span>
                    <span className="text-orange-400 font-medium">{currentTrack.artist}</span>
                  </div>
                </div>
              </div>
            ) : (
      ...
      >
        {isAdActive && currentAd ? (
          <div className="max-w-7xl mx-auto rounded-2xl border border-amber-500/50 bg-[#140c03]/95 text-white backdrop-blur-2xl shadow-[0_10px_40px_rgba(245,158,11,0.25)] px-3 sm:px-5 py-2.5 flex items-center justify-between gap-3 animate-in fade-in duration-200">
            {/* Left: Ad Cover & Info */}
            <div
              onClick={() => setExpanded(true)}
              className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer group"
            >
              <div className="relative h-11 w-11 sm:h-13 sm:w-13 shrink-0 rounded-xl overflow-hidden shadow-md border border-amber-500/40 bg-black">
                <Image
                  src={currentAd.imageUrl || currentAd.creativeUrl || "/recommendations/thumbnails/1.jpg"}
                  alt={currentAd.title}
                  fill
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5 mb-0.5">
                  <span className="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-500 text-black">
                    Ad Break
                  </span>
                  <span className="text-[10px] text-amber-300/80 font-semibold truncate">
                    Track #{sessionTrackCount}
                  </span>
                </div>
                <p className="text-xs sm:text-sm font-bold text-white truncate group-hover:text-amber-400 transition">
                  {currentAd.title}
                </p>
                <p className="text-[11px] font-medium text-slate-400 truncate">
                  {currentAd.sponsorName || "Sponsored"}
                </p>
              </div>
            </div>

            {/* Right: Visit Sponsor & Skip Button */}
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              {(currentAd.linkUrl || currentAd.clickUrl) && (
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={trackAdClick}
                  className="hidden sm:inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold transition border border-white/15"
                >
                  <span>Visit</span>
              ...
              {skipUnlocked ? (
                <button
                  onClick={skipAd}
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white text-xs font-bold shadow-[0_0_15px_rgba(249,115,22,0.4)] transition hover:scale-105 active:scale-95"
                >
                  Skip Ad →
                </button>
              ) : (
                <div className="px-3.5 py-2 rounded-xl bg-black/60 border border-white/15 text-slate-300 text-xs font-semibold flex items-center gap-1.5">
