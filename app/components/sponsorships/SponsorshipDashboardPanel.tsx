...
import { Loader2, AlertTriangle, Eye, MousePointerClick, Clock, CheckCircle2, XCircle, Ban } from "lucide-react";
...
const STATUS_META: Record<string, { label: string; cls: string; Icon: typeof Clock }> = {
  pending_payment: { label: "Awaiting payment", cls: "bg-amber-500/15 text-amber-300 light:text-amber-700", Icon: Clock },
  awaiting_assets: { label: "Awaiting your assets", cls: "bg-sky-500/15 text-sky-300 light:text-sky-700", Icon: Clock },
  active: { label: "Live", cls: "bg-emerald-500/15 text-emerald-300 light:text-emerald-700", Icon: CheckCircle2 },
  expired: { label: "Expired", cls: "bg-white/10 light:bg-black/10 text-slate-300 light:text-slate-600", Icon: XCircle },
  cancelled: { label: "Cancelled", cls: "bg-red-500/15 text-red-300 light:text-red-700", Icon: Ban },
};
...
// The real list-of-campaigns view — every sponsorship a signed-in user has
// ever purchased, with real impressions/clicks once live. Deliberately a
// standalone component with its own data fetching (not props-driven) so it
// can be dropped into two places unchanged: the "My Dashboard" tab inside
// /sponsorships (see app/sponsorships/page.tsx) and the standalone
// /sponsorships/dashboard route (kept working for anyone with that link
// already bookmarked/shared — see app/sponsorships/dashboard/page.tsx).
export default function SponsorshipDashboardPanel() {
  ...
  return (
    <div>
      {error && (
        <div className="mb-4 flex items-start gap-2 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-sm font-semibold text-red-300 light:text-red-800">
            ...
            return (
              <div key={row.sponsorshipId} className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-md transition-all duration-300 hover:border-white/20 sm:p-5 light:border-black/10 light:bg-black/5 light:hover:border-black/20">
                ...
                {row.status === "pending_payment" && (
                  <p className="mt-2 text-[11px] text-amber-300 light:text-amber-700">Payment not confirmed yet — this may just need a moment.</p>
                )}
                {row.status === "awaiting_assets" && (
                  <p className="mt-2 text-[11px] text-sky-300 light:text-sky-700">
