...
export default function PublicProfilePage() {
  ...
  return <main className="mx-auto max-w-[1440px] px-3 py-4 sm:px-5 sm:py-7 lg:px-7">
    ...
    {profile.gated ? <div className="mt-7 flex flex-col items-center justify-center rounded-3xl border border-dashed border-white/15 py-16 text-center light:border-black/15">{profile.usernamePrivacy === "private" ? <Lock size={28} className="mb-3 text-slate-500" /> : <UserCheck size={28} className="mb-3 text-slate-500" />}<p className="font-semibold text-white light:text-slate-900">{profile.usernamePrivacy === "private" ? "This account is private" : "This account is only visible to connections"}</p><p className="mt-1 max-w-xs text-sm text-slate-400 light:text-slate-600">{profile.usernamePrivacy === "private" ? `Only @${profile.username} can see their channel.` : "Follow each other (mutual In-Family) to see this channel."}</p></div> : <>
      <section className="mt-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-black uppercase tracking-[0.22em] text-orange-300 light:text-orange-700">Channel library</p><h2 className="mt-1 text-2xl font-black text-white light:text-slate-900">Videos</h2></div><SortControls sort={sort} onChange={updateSort} /></div><label className="mt-4 flex max-w-md min-w-0 items-center gap-2 rounded-2xl border border-white/10 bg-black/15 px-3 py-2 text-sm text-slate-400 focus-within:border-orange-400/40 light:border-black/10 light:bg-white/40"><Search size={16} className="text-orange-400" /><input value={searchQuery} onChange={(event) => { setSearchQuery(event.target.value); setVisibleCount(12); }} placeholder="Search this channel" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-slate-500 light:text-slate-900" /></label>{videos.length ? <><div className="mt-5 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{videos.slice(0, visibleCount).map((video) => <HomeVideoCard key={video.videoId} video={video} />)}</div>{visibleCount < videos.length && <div className="mt-6 flex justify-center"><button onClick={() => setVisibleCount((count) => count + 12)} className="rounded-full border border-orange-400/30 bg-orange-500/10 px-5 py-2.5 text-sm font-bold text-orange-200 transition hover:bg-orange-500/20 light:text-orange-700">Load more videos</button></div>}</> : <EmptyCollection label="videos" />}</section>
      <section className="mt-10"><div className="mb-3"><p className="text-[11px] font-black uppercase tracking-[0.22em] text-orange-300 light:text-orange-700">Quick plays</p><h2 className="mt-1 text-2xl font-black text-white light:text-slate-900">Shorts</h2></div>{shorts.length ? <ShortsShelf items={shorts} /> : <EmptyCollection label="Shorts" />}</section>
      <section className="mt-10 rounded-3xl border border-white/[0.09] bg-white/[0.035] p-6 backdrop-blur-xl light:border-black/[0.09] light:bg-black/[0.025]"><p className="text-[11px] font-black uppercase tracking-[0.22em] text-orange-300 light:text-orange-700">About</p><p className="mt-4 max-w-3xl whitespace-pre-wrap text-sm leading-7 text-slate-300 light:text-slate-700">{profile.description?.trim() || "No channel description provided."}</p>{socialLinks.length > 0 && <div className="mt-7"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-orange-300 light:text-orange-700">Elsewhere</p><div className="mt-3 flex flex-wrap gap-2">{socialLinks.map((link) => <a key={`${link.label}-${link.url}`} href={link.url} target="_blank" rel="noopener noreferrer nofollow" className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-bold text-slate-200 transition hover:border-orange-400/40 hover:text-orange-300 light:border-black/10 light:text-slate-700">{link.label}</a>)}</div></div>}</section>
    </>}
  </main>;
}
